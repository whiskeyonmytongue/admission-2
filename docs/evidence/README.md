...
- `images/clone-pull.png`: 별도 clone의 push와 원본의 fast-forward pull 화면

텍스트 기록만으로 꾸미지 않고, 실행 환경과 Git 이력을 함께 확인할 수 있도록 구성합니다.

처음 세 화면은 프로젝트 루트에서 `make env`, `make demo`, `make git`으로 다시
만들 수 있습니다. `make demo`는 임시 상태 파일을 사용하고 실행 후 자동으로
정리하므로 제출용 `state.json`을 바꾸지 않습니다. 원격 왕복은
`logs/clone-pull.txt`의 실제 명령과 커밋 해시로 확인합니다.
