...
같은 객체 안에 중복 JSON 키가 있으면 어느 값을 선택할지 임의로 결정하지 않고
손상 파일로 처리합니다. 문제·선택지·힌트에는 터미널 출력을 조작할 수 있는
제어 문자나 UTF-8로 출력할 수 없는 Unicode surrogate를 저장하지 않습니다.
플레이 시각은 `T` 구분자와 시간대가 있는 ISO 8601 형식만 허용하며, 거부된
입력 때문에 프로그램이 중단되지 않습니다.
...
이력을 보여 줍니다. `make verify`는 정확히 Python 3.10에서만 실행되며 전체
Python 파일을 자동으로 찾아 문법을 컴파일한 뒤 스타일, 단위 테스트, 임시
상태를 이용한 CLI 안전 종료를 차례로 확인합니다.
시연과 테스트는
